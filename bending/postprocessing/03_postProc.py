...
# obtain node label from node set
nLeftNodes = len(myAsm.nodeSets[leftEndNodesName].nodes[0])
nRightNodes = len(myAsm.nodeSets[rightEndNodesName].nodes[0])
leftEndNodesLabel = [myAsm.nodeSets[leftEndNodesName].nodes[0][i].label for i in range(nLeftNodes)]
rightEndNodesLabel = [myAsm.nodeSets[rightEndNodesName].nodes[0][i].label for i in range(nRightNodes)]
# initialize node set 
leftEndNodesIdx = [-1]*len(leftEndNodesLabel)
rightEndNodesIdx = [-1]*len(rightEndNodesLabel)
leftEndNodesCoordIdx = [-1]*len(leftEndNodesLabel)
rightEndNodesCoordIdx = [-1]*len(rightEndNodesLabel)
# convert label to idx
for i, lbl in enumerate(leftEndNodesLabel):
    leftEndNodesIdx[i] = int(updateNodeLabelToIdx(lbl, myStep.frames[0].fieldOutputs['U'].values))
for i, lbl in enumerate(rightEndNodesLabel):
    rightEndNodesIdx[i] = int(updateNodeLabelToIdx(lbl, myStep.frames[0].fieldOutputs['U'].values))
for i, lbl in enumerate(leftEndNodesLabel):
    leftEndNodesCoordIdx[i] = int(updateNodeCoordLabelToIdx(lbl, myInstance))
for i, lbl in enumerate(rightEndNodesLabel):
    rightEndNodesCoordIdx[i] = int(updateNodeCoordLabelToIdx(lbl, myInstance))
# # # determine force-disp and angle-disp
yForce = np.empty((nFrames))
yDisp = np.empty((nFrames))
angle = np.empty((nFrames))
for fr in range(0, nFrames):
    ...
    yForce[fr] = -2*myFrame.fieldOutputs['RF'].getSubset(region=myAsm.nodeSets['REFPUNCH']).values[0].data[1]/1e9
    angle[fr] = getBendingAngle(leftEndNodesIdx, rightEndNodesIdx, 
                            leftEndNodesCoordIdx, rightEndNodesCoordIdx, myFrame)
AD = np.c_[yDisp, angle]
session.XYData(data=AD.tolist(), name='angle')
FD = np.c_[yDisp, yForce] # prepare to be written in the output file
session.XYData(data=FD.tolist(), name='force-disp')
FDTitle = 'Disp,Force,Angle' # header to be filled in the output file
# # # analyze element
...
if 'localVal' in locals():
    listVal = np.c_[FD, angle, localVal].tolist()
