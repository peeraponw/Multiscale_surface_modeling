...
boxsize = 30        # in plate thickness direction
boxwidth = 22       # in circumferential direction
boxdepth = 30       # in plate radial direction
...
# sketch part
s.rectangle(point1=(-0.5*boxwidth, -0.5*boxdepth), point2=(0.5*boxwidth, 0.5*boxdepth))
...
face_i = (p.faces.getByBoundingBox(
    xMin=-boxwidth, xMax=boxwidth, 
    yMin=-boxdepth, yMax=boxdepth,
    zMin=-boxsize, zMax=0))
boxFaces_list.append(face_i)
# right face
face_i = (p.faces.getByBoundingBox(
    xMin=boxwidth/2., xMax=boxwidth, 
    yMin=-boxdepth, yMax=boxdepth,
    zMin=-boxsize, zMax=boxsize))
boxFaces_list.append(face_i)
# left face
face_i = (p.faces.getByBoundingBox(
    xMin=-boxwidth, xMax=-boxwidth/2., 
    yMin=-boxdepth, yMax=boxdepth,
    zMin=-boxsize, zMax=boxsize))
boxFaces_list.append(face_i)
# bottom face
...
p.DatumCsysByThreePoints(coordSysType=CARTESIAN, name='partCsys', 
    origin=(boxwidth/2,boxdepth/2,0),
    point1=(-boxwidth/2,boxdepth/2,0), 
    point2=(boxwidth/2,boxdepth/2,boxsize)
    )
...
partAsm.translate(vector=nodesTopCoord[extPlane] - (boxwidth/2., boxdepth/2., 0))
...
p.seedEdgeBySize(size=meshSizeTrans, edges=p.edges.getByBoundingBox(
    xMin=-boxwidth, xMax=boxwidth, 
    yMin=-transPartitionH, yMax=boxdepth,
    zMin=-boxsize, zMax=boxsize),
constraint=FINER)
# local zone
p.seedEdgeBySize(size=meshSizeLocal, edges=p.edges.getByBoundingBox(
    xMin=-boxwidth, xMax=boxwidth, 
    yMin=-localPartitionH, yMax=boxdepth,
    zMin=-boxsize, zMax=boxsize), 
constraint=FINER)
# generate mesh
