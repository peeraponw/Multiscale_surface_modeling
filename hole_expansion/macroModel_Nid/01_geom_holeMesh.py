...
symmFac = 1 # 4 for full model, 2 for half model, 1 for quater model, 0 to use revolve angle
revolveAngle = 45 # Revolution angle if symmFac == 0 (in deg)
# plate dimension
...
# # # Mesh
if symmFac >= 1:
    asmAngle = np.deg2rad(0)

# instance mesh
myAsm.seedPartInstance(size = meshSizeInstance, regions = (plateAsm, ))
# global mesh
myAsm.seedEdgeBySize(size=meshSizeGlobal, edges=plateAsm.edges.getByBoundingCylinder(
    center1=(d/2., 0, 0), 
    center2=(w, 0, 0), 
    radius=meshSizeLocal), 
    constraint=FINER)
myAsm.seedEdgeBySize(size=meshSizeGlobal, edges=plateAsm.edges.getByBoundingCylinder(
    center1=(d/2., t, 0), 
    center2=(w, t, 0), 
    ...
    constraint=FINER)
myAsm.seedEdgeBySize(size=meshSizeGlobal, edges=plateAsm.edges.getByBoundingCylinder(
    center1=(d/2.*np.sin(asmAngle), 0, d/2.*np.cos(asmAngle)), 
    center2=(w*np.sin(asmAngle), 0, w*np.cos(asmAngle)), 
    radius=meshSizeLocal), 
    constraint=FINER)
myAsm.seedEdgeBySize(size=meshSizeGlobal, edges=plateAsm.edges.getByBoundingCylinder(
    center1=(d/2.*np.sin(asmAngle), t, d/2.*np.cos(asmAngle)), 
    center2=(w*np.sin(asmAngle), t, w*np.cos(asmAngle)), 
    radius=meshSizeLocal), 
    constraint=FINER)
# transition mesh
myAsm.seedEdgeBySize(size=meshSizeTrans, edges=plateAsm.edges.getByBoundingCylinder(
    center1=(d/2., 0, 0), 
    center2=(transMeshD/2.+eps, 0, 0), 
    radius=meshSizeLocal), 
    constraint=FINER)
myAsm.seedEdgeBySize(size=meshSizeTrans, edges=plateAsm.edges.getByBoundingCylinder(
    center1=(d/2., t, 0), 
    center2=(transMeshD/2.+eps, t, 0), 
    radius=meshSizeLocal), 
    constraint=FINER)
myAsm.seedEdgeBySize(size=meshSizeTrans, edges=plateAsm.edges.getByBoundingCylinder(
    center1=(d/2.*np.sin(asmAngle), 0, d/2.*np.cos(asmAngle)), 
    center2=((transMeshD/2.+eps)*np.sin(asmAngle), 0, (transMeshD/2.+eps)*np.cos(asmAngle)), 
    radius=meshSizeLocal), 
    constraint=FINER)
myAsm.seedEdgeBySize(size=meshSizeTrans, edges=plateAsm.edges.getByBoundingCylinder(
    center1=(d/2.*np.sin(asmAngle), t, d/2.*np.cos(asmAngle)), 
    center2=((transMeshD/2.+eps)*np.sin(asmAngle), t, (transMeshD/2.+eps)*np.cos(asmAngle)), 
    radius=meshSizeLocal), 
    constraint=FINER)
# local mesh
myAsm.seedEdgeBySize(size=meshSizeLocal, edges=plateAsm.edges.getByBoundingCylinder(
    center1=(d/2., 0, 0), 
    center2=(localMeshD/2.+eps, 0, 0), 
    radius=meshSizeLocal), 
    constraint=FINER)
myAsm.seedEdgeBySize(size=meshSizeLocal, edges=plateAsm.edges.getByBoundingCylinder(
    center1=(d/2., t, 0), 
    center2=(localMeshD/2.+eps, t, 0), 
    radius=meshSizeLocal), 
    constraint=FINER)
myAsm.seedEdgeBySize(size=meshSizeLocal, edges=plateAsm.edges.getByBoundingCylinder(
    center1=(d/2.*np.sin(asmAngle), 0, d/2.*np.cos(asmAngle)), 
    center2=((localMeshD/2.+eps)*np.sin(asmAngle), 0, (localMeshD/2.+eps)*np.cos(asmAngle)), 
	radius=meshSizeLocal), 
	constraint=FINER)
myAsm.seedEdgeBySize(size=meshSizeLocal, edges=plateAsm.edges.getByBoundingCylinder(
    center1=(d/2.*np.sin(asmAngle), t, d/2.*np.cos(asmAngle)), 
    center2=((localMeshD/2.+eps)*np.sin(asmAngle), t, (localMeshD/2.+eps)*np.cos(asmAngle)), 
    radius=meshSizeLocal), 
    constraint=FINER)
# hole mesh
myAsm.seedEdgeBySize(size=meshSizeHole, edges=plateAsm.edges.getByBoundingCylinder(
    center1=(0,0,0), center2=(0,t+eps,0), radius=d/2.+eps), constraint=FINER)
