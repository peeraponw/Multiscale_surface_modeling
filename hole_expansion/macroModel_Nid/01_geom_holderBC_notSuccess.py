...
revolveAngle = 5 # Revolution angle if symmFac == 0 (in deg)
angleRatio = 0.1 # BC void strip between fixed holder and symmFac region (value around 0.1-0.2)
# plate dimension
...
# create set for holder
platePart.Set(name='freePlate', cells=platePart.cells.getByBoundingCylinder(
    center1=(0,0,0), center2=(0,0,t), radius=holderD/2.))
platePart.SetByBoolean(name='fixedPlate', 
    sets=(platePart.sets['wholePlate'], platePart.sets['freePlate']),
    operation=DIFFERENCE)
fixedPlate = platePart.sets['fixedPlate']


# partition for holder BC
if symmFac == 0:
    platePart.DatumPlaneByPrincipalPlane(offset = 0, principalPlane = XZPLANE)
    planeKey = platePart.datums.keys()[-1]
    platePart.DatumAxisByTwoPoint(point1=(0,0,0), point2=(0,0,w))
    axisKey = platePart.datums.keys()[-1]

    platePart.DatumPlaneByRotation(plane=platePart.datums[planeKey], axis=platePart.datums[axisKey], angle=0.5) 
    platePart.DatumPlaneByRotation(plane=platePart.datums[planeKey], axis=platePart.datums[axisKey], angle=4.5) 

    platePart.PartitionCellByDatumPlane(cells = fixedPlate.cells, datumPlane = platePart.datums[planeKey+2])
    platePart.PartitionCellByDatumPlane(cells = fixedPlate.cells, datumPlane = platePart.datums[planeKey+3])

    # ## Create set for fixed BC
    # # top
    platePart.Set(name='top-fix', faces=platePart.faces.findAt((w-eps,(w-eps)*np.tan(np.deg2rad(2.5)),t)))
    # # bottom
    platePart.Set(name='bot-fix', faces=platePart.faces.findAt((w-eps,(w-eps)*np.tan(np.deg2rad(2.5)),0)))
