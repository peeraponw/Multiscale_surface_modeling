...
C -----------------------------------------------------------C
C               Modifications by Felix Pütz                  C
C Switches for Temperature and Strainrate                    C
C ntempSwitch -> 1 Temp dependency on; 0 off                 C
C nstrainSwitch -> 1 Strain dependency on ; 0 off            C
C -----------------------------------------------------------C
C               Modifications by David Lenz                  C
C Switches for Dcrit-Locus, damage and brittle fracture      C
C ndcritlocusSwitch -> 1 dcrit-locus on; 0 dcrit on and      C
C										 dcrit-locus off     C
C ndamageSwitch -> 1 damage on; 0 off                        C
C nbrittleSwitch -> 1 brittle fracture on ; 0 off            C
C -----------------------------------------------------------C
C               Modifications by Warm
C Create a proportional DIL shifting scheme 
C due to surface condition.
C 
C surfLocalized -> float between 0 to 1 ; 
C -----------------------------------------------------------C
        subroutine vumat(
C Read only
...
C       Dcrit-locus
        ...
        Dc3     = props(31)
        Dc4     = props(32)
C		PROPS(33)=Switch temerapture function on(1)/of(0)
C  		PROPS(34)=Switch strain function on(1)/of(0)
		ntempSwitch=PROPS(33)
		nstrainSwitch=PROPS(34)
C		PROPS(35)=Switch Dcrit-locus on(1)/of(0)
		ndcritlocusSwitch=PROPS(35)
C		PROPS(36)=Switch Damage on(1)/of(0)
		ndamageSwitch=PROPS (36)
C		PROPS (37)=Switch brittle fracture on(1)/of(0)
		nbrittleSwitch=PROPS (37)
...
C Check if surface factor in material parameter file 
C is updated to the version considered surface.
        if (surfFactor.eq.zero) then
          surfFactor = 1.0
        endif
C
...
C If brittle damage has previously occured, stress is zeroed
		if (stateOld(i,4).gt.half .and. stateOld(i,4).lt.(one+half) .and.
     1		nbrittleSwitch.NE.zero) then
          stressNew(i,1) = zero
          stressNew(i,2) = zero
          stressNew(i,3) = zero
          stressNew(i,4) = zero
...
C Restoring previous softening correction term due to ductile damage
		if (ndamageSwitch.NE.zero) THEN
        facDctlDmgDev = one - stateOld(i,2)
		elseif (ndamageSwitch.EQ.zero) THEN
		facDctlDmgDev = one
		endif
...
C         Updating failure indicator and damage fraction variable
		if (ndcritlocusSwitch.NE.zero) THEN 
         Dcritl=(Dc1*exp(zero-Dc2*eta) - Dc3*exp(zero-Dc4*eta) )
     1         * thetabar**2  +  Dc3*exp(zero-Dc4*eta)
		  stateNew(i,34)= Dcritl
...
C       Calculating new softening correction terms due to ductile damage
        stateNew(i,28)= one
        if (stateNew(i,32).ge.one .and. ndamageSwitch.NE.zero) then
          facDctlDmgDev = zero
          facDctlDmgVol = zero
          stateNew(i,32) = one
          stateNew(i,28)= zero
        elseif (stateNew(i,32).ne.one .and. ndamageSwitch.NE.zero) then
          facDctlDmgDev = one - stateNew(i,2)
          facDctlDmgVol = facDctlDmgDev 
        endif
C		  DAMAGE OFF		 
		if (stateNew(i,32).ge.one .and. ndamageSwitch.EQ.zero) then
...
C         stateNew(i,28)= zero
        elseif (stateNew(i,32).ne.one .and. ndamageSwitch.EQ.zero) then
C			DAMAGE OFF
...
C       Brittle damage
		if (sigmamax.ge.sigdmg .and. stressNew(i,1).gt.zero .and.
     1		nbrittleSwitch.NE.zero) then
          stressNew(i,1) = zero
          stressNew(i,2) = zero
          stressNew(i,3) = zero
